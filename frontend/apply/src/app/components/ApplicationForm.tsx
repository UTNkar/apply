...
export default function ApplicationForm({ mode, position, existingApplication }: ApplicationFormProps) {
  const router = useRouter()
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [referenceErrors, setReferenceErrors] = useState<ReferenceErrors[]>([])

  const [coverLetter, setCoverLetter] = useState(existingApplication?.cover_letter || '')
  const [qualifications, setQualifications] = useState(existingApplication?.qualifications || '')
  const [gdpr, setGdpr] = useState(existingApplication?.gdpr || false)
  const [references, setReferences] = useState<Reference[]>([])

  const isEditable = mode === 'create' || existingApplication?.status === 'draft'

  // Load existing references when editing
  useEffect(() => {
    if (existingApplication?.references && existingApplication.references.length > 0) {
  ...
  const addReference = () => {
    if (references.length < 3) {
      setReferences([...references, { name: '', phone_num: '', email: '', comment: '', title: '' }])
    }
  }
  ...
  const handleSubmit = async (status: 'draft' | 'submitted') => {
    ...
    try {
      ...
      const error = err as { message?: string; fieldErrors?: { references?: ReferenceErrors[] } }
  ...
  return (
    <div className="pageContainer">
      ...
      <div className={styles.infoCard}>
        <p><strong>Team:</strong> {position.role.team_name}</p>
        <p><strong>Application Deadline:</strong> {formatDate(position.recruitment_end)}</p>
        <p><strong>Term of office:</strong> {formatDate(position.term_from)} — {formatDate(position.term_end)}</p>

        {mode === 'edit' && existingApplication && (
          <p>
            <strong>Status:</strong>{' '}
            <span className={`${styles.statusBadge} ${existingApplication.status === 'draft' ? styles.draft : styles.submitted}`}>
        ...
        {position.role.description && (
          <>
            <p style={{ marginTop: '1rem' }}><strong>Role Description:</strong></p>
        ...
        {position.comment && (
          <>
            <p style={{ marginTop: '1rem' }}><strong>Comments for this year:</strong></p>
              ...
              <div className={styles.formFieldRow}>
                <div className={styles.formFieldWithIcon}>
                  ...
                  {referenceErrors[index]?.name && (
                    <p className={styles.fieldError}>{referenceErrors[index].name[0]}</p>
                  )}
                </div>

                <div className={styles.formFieldWithIcon}>
                  <label className={styles.formLabel}>Title/role e.g. Boss</label>
                  <div style={{ position: 'relative' }}>
                    <span className={styles.inputIcon}>
                      <svg fill="currentColor" viewBox="0 0 24 24">
                ...
                <div className={styles.formFieldWithIcon}>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input
                      type="tel"
                      value={ref.phone_num}
                      onChange={(e) => updateReference(index, 'phone_num', e.target.value)}
                  ...
                  <p className={styles.fieldError}>{referenceErrors[index].email_or_phone_num![0]}</p>
                )}
                </div>
        ...
        <div className={styles.formSection}>
          ...
          <div className={styles.checkboxContainer}>
            ...
            <span className={styles.checkboxLabel}>
              I accept that my data is saved in accordance with Uppsala Union of Engineering
              and Science Students integrity policy that can be found within the link:{' '}
              <a href="https://utn.se/dokumentarkiv" target="_blank" rel="noopener noreferrer">
          ...
          {isEditable ? (
            <>
              <button
                ...
                disabled={submitting || !coverLetter || !qualifications || !gdpr}
